...
import type { RequestItemInterface } from "../interfaces/RequestItemInterface";
...
export async function createRequest(
  ...
  if (!requestItems || requestItems.length === 0) {
    throw new InvalidPayloadError(
      "Error creating request: Request must contain at least one item",
    );
  }
  if (requestItems.some((item) => item.quantity <= 0)) {
    throw new InvalidPayloadError(
      "Error creating request: All items must have a quantity greater than zero",
    );
  }
...
export async function submitRequest(id: string, userId: string) {
  ...
  if (request.status !== ApprovalStatus.draft)
    throw new InvalidPayloadError("Request must be a draft to be submitted.");
...
export async function update(id: string, items: RequestItemInterface[]) {
  if (!items.length)
    throw new InvalidPayloadError("Items array cannot be empty");
  const request = await getRequestById(id);

  if (!request) throw new NotFoundError("Not Found");

  if (request.status !== ApprovalStatus.draft)
    throw new InvalidPayloadError("Only draft requests can be updated.");

  // biome-ignore lint/style/noNonNullAssertion: <not-null assertion por conta do check realizado acima>
