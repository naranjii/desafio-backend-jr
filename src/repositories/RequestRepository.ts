...
export const RequestRepository = {
    async create({ userId, requestItems }: CreateRequestInterface) {
        return prisma.purchaseRequest.create({
            data: {
                userId, status: ApprovalStatus.draft, history: {
                    create: {
    ...
    async submit(data: SubmitRequestInterface) {
        return prisma.purchaseRequest.update({
            where: { id: data.id },
            data: {
                status: ApprovalStatus.submitted, history: {
                    create: {
    ...
    async reject(data: UpdateStatusInterface) {
        return prisma.purchaseRequest.update({
            where: { id: data.id },
            data: {
                status: ApprovalStatus.rejected, history: {
                    create: {
    ...
    async approve(data: UpdateStatusInterface) {
        return prisma.purchaseRequest.update({
            where: { id: data.id },
            data: {
                status: ApprovalStatus.approved, history: {
                    create: {
                        fromStatus: data.previousStatus,
